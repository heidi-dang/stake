...
def calculate_ema_cross_bet(balance):
    ...
    if ema5 is not None and ema20 is not None:
        if ema5 > ema20:
            condition = "over"; target = 50.50
        else:
            condition = "under"; target = 49.50
            
    return max(cfg['min_bet_floor'], min(base_bet, balance * 0.003)), condition, target
...
def is_locked():
    ...
    else: daily_cap = cfg['eternal_daily_loss_cap_pct']
    ...
    while True:
        with state_lock:
            ...
            if state['strategy'] == 'custom':
                ...
                current_bet = calculate_basic_bet(state['balance']['available'])
                condition = state['config'].get('basic_condition', 'over')
                target = state['config'].get('basic_target', 50.50)
        ...
        with state_lock:
            ...
            if won:
                ...
                if state['strategy'] == 'basic':
                    ...
                    if win_action == 'reset': state['basic_current_bet'] = float(cfg.get('basic_bet_amount', 0.000001))
                    elif win_action == 'multiply': state['basic_current_bet'] *= float(cfg.get('basic_win_mult', 1.0))
                ...
                if state['strategy'] == 'basic':
                    ...
                    if loss_action == 'reset': state['basic_current_bet'] = float(cfg.get('basic_bet_amount', 0.000001))
                    elif loss_action == 'multiply': state['basic_current_bet'] *= float(cfg.get('basic_loss_mult', 2.0))
    ...
    <script>
        window.onerror = function(msg, url, line, col, error) {
            fetch('/js_error', {
                ...
                body: JSON.stringify({msg: msg, line: line, stack: error ? error.stack : ''})
            });
            return false;
        };
        ...
        <div class="panel">
            ...
            <div id="basic_config" class="strat-panel" style="display:none;">
                <div class="config-grid">
                    ...
                    <div id="basic_win_mult_div" class="form-group" style="display:none;">
                    ...
                    <div id="basic_loss_mult_div" class="form-group">
                    ...
                    <div class="form-group"><label>Target Number</label><input id="basic_target" type="number" step="0.01" value="50.50" oninput="syncWinChance('')"></div>
                    <div class="form-group">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:2rem;">
            ...
            <div style="display:flex; flex-direction:column; gap:2rem;">
                <div class="panel">
                    ...
                    <!-- Basic Strategy Panel (Simulation) -->
                    <div id="sim_basic_config" class="sim-strat-panel" style="display:none;">
                        <div class="config-grid">
                            ...
                            <div id="s_basic_win_mult_div" class="form-group" style="display:none;">
                                <label>Win Multiplier</label>
                            ...
                            <div id="s_basic_loss_mult_div" class="form-group">
                                <label>Loss Multiplier</label>
                            ...
                            <div class="form-group"><label>Target Number</label><input id="s_basic_target" type="number" step="0.01" value="50.50" oninput="syncWinChance('s_')"></div>
                            <div class="form-group">
                                <label>Condition</label>
    ...
    <script>
        ...
        function toggleStrat() {
            ...
            if (strat === 'the_gork') {
                ...
                desc.textContent = "A simple strategy that places an absolute bet amount. Tunable win/loss multipliers.";
            }
        }
        ...
        function startBot() {
            ...
            if (strat === 'the_gork') {
                ...
                data.c_base = parseFloat(document.getElementById('c_base').value);
                data.c_tp = parseFloat(document.getElementById('c_tp').value);
                data.c_sl = parseFloat(document.getElementById('c_sl').value);
        ...
        function simToggleStrat() {
            ...
            const map = { the_gork:'sim_gork_config', basic:'sim_basic_config', ema_cross:'sim_ema_config', die_last:'sim_die_last_config', vanish_in_volume:'sim_vanish_config', eternal_volume:'sim_eternal_config', custom:'sim_custom_config' };
        ...
        function getSimParams() {
            ...
            if (strat === 'the_gork') {
                ...
                data.c_base = parseFloat(document.getElementById('s_c_base').value);
                data.c_tp = parseFloat(document.getElementById('s_c_tp').value);
                data.c_sl = parseFloat(document.getElementById('s_c_sl').value);
        ...
        function toggleBasicMults() {
            ['', 's_'].forEach(prefix => {
                ...
                const winDiv = document.getElementById(prefix + 'basic_win_mult_div');
                ...
                const lossDiv = document.getElementById(prefix + 'basic_loss_mult_div');
        ...
        function renderDynamicParams(parent_id, container_id, prefix = '') {
            ...
            fetch('/strategy/params', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('gork_jwt') } })
                .then(r => r.json())
                .then(params => {
                    container.innerHTML = '';
...
def js_error():
    data = request.json or {}
    logger.error(f"FRONTEND JS ERROR: {data.get('msg')} at line {data.get('line')}\n{data.get('stack')}")
...
def calculate_basic_bet(balance):
    cfg = state['config']
    if 'basic_current_bet' not in state:
        state['basic_current_bet'] = float(cfg.get('basic_bet_amount', 0.000001))
    return state['basic_current_bet']
...
def _run_simulation_internal(data):
    ...
    for _ in range(beats_to_sim):
        ...
        if (strat == 'custom'): bet = execute_custom_strategy(sim_bal, state)
            ...
            bet = calculate_basic_bet(sim_bal)
            condition = state['config'].get('basic_condition', 'over')
            target = state['config'].get('basic_target', 50.50)
        ...
        if won:
            ...
            # SIM BASIC WIN LOGIC
            if strat == 'basic':
                win_action = state['config'].get('basic_on_win', 'reset')
                if win_action == 'reset': state['basic_current_bet'] = float(state['config'].get('basic_bet_amount', 0.000001))
                elif win_action == 'multiply': state['basic_current_bet'] *= float(state['config'].get('basic_win_mult', 1.0))
            ...
            if strat == 'basic':
                loss_action = state['config'].get('basic_on_loss', 'multiply')
                if loss_action == 'reset': state['basic_current_bet'] = float(state['config'].get('basic_bet_amount', 0.000001))
                elif loss_action == 'multiply': state['basic_current_bet'] *= float(state['config'].get('basic_loss_mult', 2.0))
...
def simulate():
    try:
        ...
        # _run_simulation_internal returns a Flask Response object from jsonify
        return results
    except Exception as e:
...
- [WRITE_STRATEGY]: Start writing a new custom Python strategy. 
  ...
  The code MUST define a function `calculate_bet(balance)` and set `result = calculate_bet(balance)`.
...
"""
    ...
    try:
        ...
        if 'candidates' not in r_json:
            err = r_json.get('error', {}).get('message', 'Unknown API Error')
